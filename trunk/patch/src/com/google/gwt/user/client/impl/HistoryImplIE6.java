 ...
 * event, and for which {@link com.google.gwt.user.client.impl.HistoryImplTimer} will not work either.
 */
class HistoryImplIE6 extends HistoryImpl {
  ...
  // TODO(alexe): it makes more sense to create the frame here instead of requiring that it be embedded into the hostpage, because only HistoryImplIE6 seems to use it
  // NOTE(alexe): some of this code was copied from IFrameTemplate.js used by IFrameLinker
  // TODO(alexe): file a ticket for this, test this in IE6/7, and then contribute the patch
  private static native Element getOrCreateHistoryFrame() /*-{
    var id = '__gwt_historyFrame', iframe = $doc.getElementById(id);
    if (!iframe) {
      ...
      // We have to use the location.replace trick to avoid Firefox soft refresh quirks.
  ...
  @Override
  public boolean init() {
    historyFrame = getOrCreateHistoryFrame();
