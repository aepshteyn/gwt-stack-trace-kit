...
public abstract class SelectionScriptLinker extends AbstractLinker {
   ...
   * @param sourceMapsEnabled    Splitting the script could affect the source maps
   ...
   * @param context
   */
  public static String splitPrimaryJavaScript(StatementRanges ranges, boolean sourceMapsEnabled, String js,
                                              int charsPerChunk, String scriptChunkSeparator, LinkerContext context) {
    // TODO(cromwellian) enable chunking with sourcemaps
    if (charsPerChunk < 0 || ranges == null || sourceMapsEnabled) {
      return js;
    }
  ...
  protected byte[] generatePrimaryFragment(TreeLogger logger,
    ...
    String temp = splitPrimaryJavaScript(result.getStatementRanges()[0],
        !artifacts.find(SymbolMapsLinker.SourceMapArtifact.class).isEmpty(), js[0],
