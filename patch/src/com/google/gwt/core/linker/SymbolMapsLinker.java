 ...
 * value returned by {@link com.google.gwt.core.ext.linker.CompilationResult#getStrongName()}.
 */
...
public class SymbolMapsLinker extends AbstractLinker {
  ...
  /**
   * Artifact to represent a text file containing a key to decode filenames
   * obfuscated by FileNameObfuscator.
   */
  public static class FilenameDeobfuscationArtifact extends Artifact<FilenameDeobfuscationArtifact> {
    private final int permutationId;
    /** This will look like "Foo.java,Bar.java,...etc" */
    private String filenames;

    public FilenameDeobfuscationArtifact(int permutationId, String filenames) {
      super(SymbolMapsLinker.class);
      this.permutationId = permutationId;
    ...
    protected int compareToComparableArtifact(SymbolMapsLinker.FilenameDeobfuscationArtifact o) {
      return filenames.compareTo(o.filenames);
    }

    protected Class<FilenameDeobfuscationArtifact> getComparableArtifactType() {
      return FilenameDeobfuscationArtifact.class;
    }
  }
  ...
  private static void printPropertyMap(PrintWriter pw,
    ...
    for (Entry<SelectionProperty, String> entry : map.entrySet()) {
      if (needsComma) {
        pw.print(" , ");
      } else {
        needsComma = true;
  ...
  public ArtifactSet link(TreeLogger logger, LinkerContext context,
    ...
    if (onePermutation) {
      ...
      for (FilenameDeobfuscationArtifact fda : artifacts.find(FilenameDeobfuscationArtifact.class)) {
        String strongName = permMap.get(fda.getPermutationId());
        String partialPath = strongName  + ".obfuscatedFilenames";
        EmittedArtifact newArtifact = emitString(logger, fda.getFilenames(), partialPath);
        newArtifact.setVisibility(Visibility.Deploy);
        artifacts.add(newArtifact);
        artifacts.remove(fda);
      }
    }
    return artifacts;
  }
   ...
   * @throws com.google.gwt.core.ext.UnableToCompleteException if an error occurs
   */
  protected void doWriteSymbolMap(TreeLogger logger, CompilationResult result,
    ...
    for (SymbolData symbol : result.getSymbolMap()) {
      if (!symbol.isMethod()) {
        // TODO(alexe): StackTraceDeobfuscator is currently unable to deal with duplicated symbols, thus we only emit method symbols
        // other alternatives are:
        // 1) modify solutions.trsoftware.gwt.stacktrace.server.StackTraceDeobfuscator.loadSymbolMap to only load method symbols
        // 2) modify JsNamer to never repeat an identifier if stack emulation is enabled (this solution will help understand JS error messages too)
        continue;
      }
